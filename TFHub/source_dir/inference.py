...
# size to match the pretrained model
HEIGHT = 768
WIDTH  = 768 
...
def _predict_using_grpc(context, instance):
    ...
    result_future = stub.Predict.future(request, 60)
    
    # detection box
    output_tensor_proto = result_future.result().outputs['detection_boxes']
    #print(output_tensor_proto)
    output_shape = [dim.size for dim in output_tensor_proto.tensor_shape.dim]
    output_np_detbox = np.array(output_tensor_proto.float_val).reshape(output_shape)
    
    # detection class
    output_tensor_proto = result_future.result().outputs['detection_classes']
    #print(output_tensor_proto)
    output_shape = [dim.size for dim in output_tensor_proto.tensor_shape.dim]
    output_np_detclass = np.array(output_tensor_proto.float_val).reshape(output_shape)
    
    # detection score
    output_tensor_proto = result_future.result().outputs['detection_scores']
    #print(output_tensor_proto)
    output_shape = [dim.size for dim in output_tensor_proto.tensor_shape.dim]
    output_np_detscore = np.array(output_tensor_proto.float_val).reshape(output_shape)
    
    
    prediction_json = {'detection_boxes': output_np_detbox.tolist(), 
                       'detection_classes': output_np_detclass.tolist(),
                       'detection_scores': output_np_detscore.tolist()
                      }
